...
import be4rjp.shootarian.weapon.actions.action.Action;
import be4rjp.shootarian.weapon.actions.action.SoundAction;
...
public class Actions {
    ...
    public static void loadAllActions() {
        Shootarian.getPlugin().getLogger().info("Loading reload actions...");
        File dir = new File("plugins/Shootarian/action");
    
        dir.getParentFile().mkdir();
        dir.mkdir();
        File[] files = dir.listFiles();
        if (files.length == 0) {
            Shootarian.getPlugin().saveResource("action/scar-h-reload.yml", false);
            files = dir.listFiles();
        }
